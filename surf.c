	...
	if(((XEvent *)e)->type == PropertyNotify) {
		...
		if(ev->state == PropertyNewValue) {
			if(ev->atom == atoms[AtomFind]) {
				...
				} else if (strstr((char*) arg.v, "en ") == (char*) arg.v && strlen((char*) arg.v) > 3) {
